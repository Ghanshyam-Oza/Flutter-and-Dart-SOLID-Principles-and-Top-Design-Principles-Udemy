...
class CSVContactsAdapter extends IContactsAdapter {
  CSVContactReader _reader = CSVContactReader();
  ...
  List<Contact> _parseContactsCSV(String contactsCSV) {
    ...
    List<String> lines = contactsCSV.split('\n');
    for (int i = 1; i < lines.length; i++) {
      List<String> values = lines[i].split(',');
      contactsList.add(Contact(
        fullName: values[0].trim(),
        email: values[1].trim(),
        friend: values[2].trim() != 'false',
